...
const onSelectMonth = (id, s) => {
  if (params_create.value) {
    router.push(`/academic-class/detail/score/show/${params_id.value}_${id}${s != undefined ? '_' + s : ''}`)
  } else {
    router.push(`/academic-class/detail/score/create/${params_id.value}_${id}${s != undefined ? '_' + s : ''}`)
  }
}
  ...
  <div>
    <VRow>
      <VCol
        ...
        sm="12"
      >
        <VCard :title="`ថ្នាក់ទី ${model?.level?.level} ឆ្នាំសិក្សា ${model?.academic_year?.name}`">
          <VDivider />
          <v-btn
            class="mt-5 mx-5"
            color="secondary"
          ...
          >
          <VCardText>
            <div class="text-h6 font-weight-bold">ជ្រើសរើសខែសម្រាប់{{ params_create ? 'បង្ហាញ' : 'ដាក់' }}ពិន្ទុ</div>
            <v-row class="mt-5">
              <v-col
                cols="12"
                md="6"
